export default class BNode{
    ...
    public findKey(key:number):number{
        let idx = 0;
    ...
    public removeFromLeaf(idx:number):void{
    ...
    public removeFromNonLeaf(idx:number){ 
    ...
    public getPred(idx:number) 
    { 
        // Keep moving to the right most node until we reach a leaf 
        let cur:BNode = this.childs[idx]; 
        while (!cur.isLeaf) 
    ...
    { 
        ...
        let child: BNode =this.childs[idx]; 
        let sibling: BNode=this.childs[idx-1]; 
    ...
    public remove(key:number):void{
        ...
        { 
            ...
            let flag:boolean = ( (idx==this.currKeys)? true : false ); 
